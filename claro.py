...
browser_service = Service(
    executable_path=(current_folder / "chromedriver" / "chromedriver.exe").__str__()
)
...
def sleep_wireless(status: bool) -> None:
    ...
    box_user = browser.find_element(
        By.XPATH, "/html/body/div[2]/div/form/div[1]/div/input"
    )
    box_user.send_keys(user)
    box_passwd = browser.find_element(
        By.XPATH, "/html/body/div[2]/div/form/div[2]/div/input"
    )
    box_passwd.send_keys(password)

    box_submit = browser.find_element(
        By.XPATH, "/html/body/div[2]/div/form/div[3]/button/div"
    )
    box_submit.click()
    sleep(5)
    ...
    if status:
        for ip_index in range(1, 10):
            ip_config = browser.find_element(
                By.XPATH,
            ...
            sleep_time = browser.find_element(
                By.XPATH,
                "/html/body/div[2]/div/div[2]/div/div/form/div[10]/div/div/select",
            )
            sleep_time.click()

            activeopt = browser.find_element(
                By.XPATH,
                "/html/body/div[2]/div/div[2]/div/div/form/div[10]/div/div/select/option[2]",
            )
            activeopt.click()
            submit_config = browser.find_element(
                By.XPATH, "/html/body/div[2]/div/div[2]/div/div/form/div[11]/button/div"
            )
            submit_config.click()
