...
if check_triangle_existence(*lados):
    if any(lado**2 == (lados[(pos + 1)%len(lados)]**2 + lados[(pos + 2)%len(lados)]**2) for pos, lado in enumerate(lados)):
    ...
    if [lado == lados[(pos + 1)%len(lados)] or lado == lados[(pos + 2)%3] for pos, lado in enumerate(lados)].count(False) == 1:
